...
public final class AppUtil {
     ...
     *
     * @param context                      上下文
     * @param containSystemAppPackageNames 系统应用包名，再次list中的系统应用不会被排除
     * @return 应用信息List
     */
    public static List<PackageInfo> getAllCustomerApps(Context context, List<String> containSystemAppPackageNames) {
        ...
        for (int i = 0; i < paklist.size(); i++) {
            ...
            if ((pak.applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) <= 0) {
                ...
                for (String containSystemAppPackageName : containSystemAppPackageNames) {
                    if (containSystemAppPackageName.equals(pak.packageName)) {
                        apps.add(pak);
                        break;
                    }
                }
        ...
        Log.i("Z-MoreApp", "size:" + apps.size());
        return apps;
    }
